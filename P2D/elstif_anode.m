%================= ELEMENT STIFFNESS MATRIX for ANODE================================
%
function [kel, rel] = elstif_anode(ncoord, nelnodes, lmncoord, coefs, elu, eldu)
    ...
    % 2*Pi*R / Pi*R^2 = 2
    % 4*Pi*R^2 / (4/3*Pi*R^3)
    svr = 3 * coefs.eps_inc_an / coefs.r_an; % m^-1;
    ...
    for intpt = 1:npoints
        ...
        ratio_mat = (coefs.eps_mat_an)^(coefs.alpha);
        ratio_inc = (coefs.eps_inc_an)^(coefs.alpha);
        ...
        rc = rc + coefs.dtco_mat * coefs.eps_mat_an * N * (N' * eldc) * w(intpt) * dt ...
            +df_eff * dNdx * (dNdx' * elc) * w(intpt) * dt ...
            -coefs.sour_c * svr * N * (N' * elj) * w(intpt) * dt;

        kcc = kcc + coefs.dtco_mat * coefs.eps_mat_an * (N * N') * w(intpt) * dt ...
        ...
        %  rs, kss
        % -------------------------------------------------------------------------------
        sig = coefs.sigan / coefs.sigan_ref;
        sig_eff = sig * ratio_inc;

        rs = rs + sig_eff * dNdx * (dNdx' * els) * w(intpt) * dt ...
            +coefs.sour_an * svr * N * (N' * elj) * w(intpt) * dt;

        kss = kss + sig_eff * (dNdx * dNdx') * w(intpt) * dt;

        ksj = ksj + coefs.sour_an * svr * (N * N') * w(intpt) * dt;
